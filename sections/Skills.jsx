...
{  name: "GitHub",
  icon: <FaGithub className="text-blue-600 h-8 w-8" />
},
{
  name: "Tailwind CSS",
    ...
    <section className="container mx-auto mt-10">
        ...
        <div className="grid grid-cols-3 mt-10 border rounded-2xl">
            {skills.map((skill, index) => (
                <div key={index} className="flex flex-col m-4 border rounded-2xl gap-3 items-center p-4">
                    {skill.icon}
                    <span className="mt-2 text-blue-600 font-semibold">{skill.name}</span>
                </div>
            ))}
        </div>
